...
export const bundle: RuleBundle = {
  en: {
    ...
    how: 'Switch Row Level Security (RLS) on for {table} and write policies that say who may read which rows — typically matching the signed-in user id against the row owner. Then re-run this check: the same request should come back empty.',
    ...
    beforeApplying: 'The same warning as above: RLS on with no policy denies everything, so write the policy in the same change and check the app still works while signed in.',
  },
  vi: {
    ...
    how: 'Bật Row Level Security (RLS) cho {table} và viết policy quy định ai được đọc dòng nào — thường là so id của user đang đăng nhập với chủ sở hữu của dòng đó. Sau đó chạy lại kiểm tra này: cùng request đó phải trả về rỗng.',
    ...
    beforeApplying: 'Cảnh báo tương tự ở trên: bật RLS mà chưa có policy thì chặn hết, nên hãy viết policy ngay trong cùng thay đổi đó và kiểm tra app vẫn chạy khi đã đăng nhập.',
  },
};
