...
export const bundle: RuleBundle = {
  en: {
    ...
    check: 'Open your Supabase dashboard → Table Editor and look at the tables list. Any table without the "RLS enabled" mark is readable by anyone holding the anon key — and the anon key is in your website’s source, by design.',
    ...
    beforeApplying: 'Turning RLS on before writing policies makes the table return nothing, and the app will look broken until the policies exist. Do both in the same sitting, and test with a signed-in account.',
    doNotApplyIf: 'the table is genuinely public reference data — a list of countries, published blog posts. Those still want RLS on with a read-only policy, not RLS off.',
    notes: {
  ...
  vi: {
    ...
    beforeApplying: 'Bật RLS trước khi viết policy sẽ khiến bảng không trả về gì, và app trông như hỏng cho tới khi có policy. Hãy làm cả hai trong cùng một lần, rồi test bằng tài khoản đã đăng nhập.',
    doNotApplyIf: 'bảng đó thật sự là dữ liệu tham chiếu công khai — danh sách quốc gia, bài blog đã đăng. Những bảng đó vẫn nên bật RLS kèm policy chỉ cho đọc, chứ không phải tắt RLS.',
    notes: {
